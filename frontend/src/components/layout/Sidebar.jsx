...
export default function Sidebar() {
  ...
  return (
    <div className="w-64 bg-[var(--bg-sidebar)] border-r border-[var(--border-color)] flex flex-col h-full shrink-0 transition-colors duration-200 relative overflow-hidden z-20">
      {/* Subtle agent glow overlay for sidebar */}
      <div 
        className="absolute inset-0 opacity-[0.03] dark:opacity-[0.05] pointer-events-none transition-colors duration-1000"
        style={{
          background: 'linear-gradient(180deg, var(--agent-primary) 0%, transparent 100%)'
        }}
      />

      {/* Header */}
      <div className="p-4 flex items-center gap-2 font-semibold text-lg border-b border-[var(--border-color)] relative z-10">
      ...
      {/* Footer Navigation */}
      <div className="p-3 border-t border-[var(--border-color)] space-y-1 relative z-10">
